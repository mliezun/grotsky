...
# Script copied from https://github.com/munificent/craftinginterpreters
...
def go_interpreter(name, tests):
    path = "build/" + name
    INTERPRETERS[name] = Interpreter(name, "go", [path], tests)
    GO_SUITES.append(name)


def rust_interpreter(name, tests):
    path = "build/" + name

    INTERPRETERS[name] = Interpreter(name, "rust", [path], tests)
            ...
            for line in file:
                ...
                if match:
                    ...
                    if not language or language == interpreter.language:
                        self.compile_errors.add(
                            "[{0}] {1}".format(match.group(3), match.group(4))
                        )
...
def main(argv):
    ...
    if argv[1] == "all":
        ...
        run_suites(GO_SUITES)
    elif argv[1] == "rust":
        run_suites(RUST_SUITES)
